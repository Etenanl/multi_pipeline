...
class _Flows:
    ...
    def __init__(self, path, flowCount, time_granularity, start=0):
        self.passcount = start
        # 存放数据集的路径
        self.path = path
        # 存放需要获取的flow数量
    ...
    def Read_dateset(self, path, flowCount):
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            for row in reader:
                ...
                if len(row) < 6:
                    continue
                elif count > flowCount + passcount:
                    break
                else:
                    self.source_data_set.append([int(row[4]), count-passcount])
                    count += 1
    ...
    # 将temp_flows按照pps排序，可以将相同的pps放到同一类里
    def Sort_Flow(self):
        # 取0号元素
        def Take_PPS(elem):
            return elem[0]
